...
![Temp alt text](__MEDIA_URL__CNX_HSPhysics_04_02_friction.jpg)<caption>For a box sliding across a floor, friction acts in the direction opposite to the velocity.</caption>
...
For example, in [Figure 04 02 wagon](Figure_04_02_wagon) (a), two children push a third child in a wagon at a constant velocity. The system of interest is the wagon plus the small child, as shown in part (b) of the figure. The two children behind the wagon exert external forces on this system (*F*1, *F*2). Friction *f* acting on the wheels is another external force acting on the system. Two more external forces act on the system: the **weight** *W* of the system pulling down and the normal force *N* of the ground pushing up. Notice that the wagon is not accelerating vertically, so Newton’s first law tells us that the normal force balances the weight. Because the wagon is moving forward at a constant velocity, the force of friction must have the same strength as the sum of the forces applied by the two children.

![Temp alt text](__MEDIA_URL__CNX_HSPhysics_04_02_wagon.jpg)<caption>(a) The wagon and rider form a </caption>
...
1. Fnet < F1 + F2 because the net force will not include the frictional force.
2. Fnet = F1 + F2 because the net force will not include the frictional force
3. Fnet < F1 + F2 because the net force will include the component of frictional force
4. Fnet = F1 + F2 because the net force will include the frictional force
